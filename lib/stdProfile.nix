...
in
{
  bud.cmds = with pkgs; {
    ...
    # Onboarding
    up = {
      writer = budUtils.runner [ installPkgs.nixos-generate-config git mercurial coreutils ];
      synopsis = "up";
    ...
    # Home-Manager
    home = {
      writer = budUtils.runner [ nixUnstable git mercurial coreutils ];
    ...
    install = {
      writer = budUtils.runner [ installPkgs.nixos-install git mercurial coreutils ];
      synopsis = "install HOST [ARGS]";
      help = "Shortcut for nixos-install";
      script = ../scripts/hosts-install.bash;
    };
    rebuild = {
      writer = budUtils.runner [ installPkgs.nixos-rebuild git mercurial coreutils ];
